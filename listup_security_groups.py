...
def collect_security_groups_data(regions: List[str] = None) -> tuple[List[Dict[str, Any]], pd.DataFrame]:
    """
    ...
    if all_filtered_data:
        ...
        # 컬럼 순서 조정
        columns_order = ['Region', 'SecurityGroupId', 'SecurityGroupName', 'VpcId', 
                        'Description', 'AnyOpenInbound', 'InboundRules', 'OutboundRules', 'Tags']
...
def save_results(raw_data: List[Dict[str, Any]], filtered_df: pd.DataFrame, output_dir: str = "data"):
    """
    ...
    filtered_json_file = os.path.join(output_dir, f"security_groups_filtered_{timestamp}.json")
...
def print_summary(filtered_df: pd.DataFrame):
    ...
    if any_open_count > 0:
        ...
        for _, sg in any_open_sgs.iterrows():
            print(f"    - {sg['SecurityGroupId']} ({sg['SecurityGroupName']}) in {sg['Region']}")
